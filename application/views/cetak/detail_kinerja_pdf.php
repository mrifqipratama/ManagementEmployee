...
if (!empty($queryp)) {
    $no = 1;
    foreach ($queryp as $a) {
        $jml_jawaban = $a->jawaban + $a->jawaban2 + $a->jawaban3 + $a->jawaban4 + $a->jawaban5 + $a->jawaban6 + $a->jawaban7 + $a->jawaban8 + $a->jawaban9 + $a->jawaban10;

        $pdf->SetFont('Times', '', 10);
        $pdf->Cell(3);
        $pdf->Cell(1, 0.6, $no++, 1, 0, 'C');
        $pdf->Cell(5, 0.6, $a->nip, 1, 0, 'C');
        $pdf->Cell(5.5, 0.6, $a->nama_pegawai, 1, 0, 'C');
        $pdf->Cell(5, 0.6, $a->nama_bulan, 1, 0, 'C');
        $pdf->Cell(4, 0.6, $jml_jawaban . ' Poin', 1, 0, 'C');
        $pdf->ln();
    }
}
$pdf->ln(1);
...
$pdf->ln(1);
$pdf->Output("Data Kinerja & Penilaian Pegawai.pdf", "I");
